...
int setaTipo(ts *p, char *tipoStr)
{
	int tipo=0;
	int subTipo=0;
	// finds type in types array
	for (int i = 0; i < taMvetorDeTipos; ++i)
	{
		if (!strcmp(tipoStr, vetorDeTipos[i].nome))
		{
			tipo = vetorDeTipos[i].valor;
			subTipo = vetorDeTipos[i].ValorsubTipo;
			break;
		}
	}
	// if no type has been found, return -1 code as an error
...
int setaTipoEDeslocamentoFuncao(ts *p, char *tipoStr)
{
	int tipo=0;
	int subTipo=0;
	// finds type in types array
