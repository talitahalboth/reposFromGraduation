...
# check is a path forms  an AB cycle
def isABcycle(path, size):
    ...
    # stores the tima at which an edge was visited and its from parent A.
...
def generateABcycle(Ea,Eb,problem, size):
    ...
    while (edgesInA > 0):
        ...
        # select U, E V s.t. number of edges incident to vertex  included in RA != 0 randomly.
        ...
        # put back in adj lists vertexes that werent used in cycle
        if (i > 0 and len(path)):
            for index in range(0,i):
                ...
                if (index%2==0):
                    ...
                    adjListB[v].append(u)
                    adjListB[u].append(v)
        # get vertexes that form cycle from the path
        cycle = []
...
def generateIntermediateSolution(D, Ea, Eb, C):
    A = 0
    B = 1
    for subTour in D:
        current = A
...
def eax (a,b,problem):
    ...
    intermediateSolution = generateIntermediateSolution(D, Ea, Eb, C)
